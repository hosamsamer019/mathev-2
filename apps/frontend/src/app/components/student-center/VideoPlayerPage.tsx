...
export default function VideoPlayerPage() {
  ...
  const initPlayer = () => {
    ...
    const match = lesson.videoUrl.match(/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|live\/|shorts\/|watch\?v=|&v=)([^#&?]*).*/);
      ...
      events: {
        onStateChange: (event: any) => {
          ...
          if (event.data === 1) {
            ...
            if (!hasAutoResumed.current && maxTimeRef.current > 0 && typeof playerRef.current?.getCurrentTime === 'function' && playerRef.current.getCurrentTime() < 2) {
               hasAutoResumed.current = true;
               if (typeof playerRef.current?.seekTo === 'function') playerRef.current.seekTo(maxTimeRef.current, true);
            }
        ...
        onError: (event: any) => {
          ...
          if (event.data === 101 || event.data === 150) {
            toast.error('صاحب هذا الفيديو على يوتيوب قام بتعطيل ميزة التضمين (Embedding). يرجى تفعيل "السماح بالتضمين" (Allow embedding) في إعدادات الفيديو على YouTube Studio.');
          } else if (event.data === 100 || event.data === 2) {
            toast.error('الفيديو غير موجود أو الرابط غير صالح.');
          } else {
            toast.error('تعذر تشغيل الفيديو من يوتيوب. تأكد من إعدادات الخصوصية والتضمين.');
          }
        }
      }
    });
  ...
  const executeProgrammaticSeek = (targetTime: number, hitQuiz: any = null) => {
    ...
    pendingSeekTicksRef.current = 0;
    pendingSeekLastTimeRef.current = typeof playerRef.current.getCurrentTime === 'function' ? (playerRef.current.getCurrentTime() || 0) : 0;
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const rate = typeof playerRef.current.getPlaybackRate === 'function' ? (playerRef.current.getPlaybackRate() || 1) : 1;
         ...
         for (const quiz of sortedQuizzes) {
            if (!quizAnswered[quiz.id]) {
               ...
               if (lastYtTimeRef.current < quiz.timestampSec + 0.1 && currentTime >= quiz.timestampSec - 0.1) {
                 if (typeof playerRef.current.pauseVideo === 'function') playerRef.current.pauseVideo();
                 if (typeof playerRef.current.seekTo === 'function') playerRef.current.seekTo(quiz.timestampSec, true);
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const currentTime = pendingSeekTargetRef.current !== null 
                        ...
                        : (typeof playerRef.current.getCurrentTime === 'function' ? (playerRef.current.getCurrentTime() || 0) : 0);
          ...
          <div className="lg:col-span-2 space-y-4">
            <div ref={containerRef} className="bg-black rounded-xl overflow-hidden relative" onContextMenu={e => e.preventDefault()}>
              <div className="relative aspect-video bg-gray-800 flex items-center justify-center">
                {lesson?.videoUrl ? (
                  isGoogleDriveUrl(lesson.videoUrl) ? (
                    ...
                    isDirectVideoUrl(lesson.videoUrl) ? (
                      ...
                      />
                    ) : lesson.videoUrl.match(/^.*(youtu\.be\/|v\/|u\/\w\/|embed\/|live\/|shorts\/|watch\?v=|&v=)([^#&?]*).*/) ? (
                      <div className="w-full h-full pointer-events-none">
                        <div id="yt-player" className="w-full h-full pointer-events-none" />
                      </div>
                    ) : (
                      <div className="w-full h-full flex flex-col pointer-events-auto z-[30]">
                        <iframe
                          src={lesson.videoUrl}
                ...
                {(!lesson?.videoUrl || (!isGoogleDriveUrl(lesson.videoUrl) && (isDirectVideoUrl(lesson.videoUrl) || !!lesson.videoUrl.match(/^.*(youtu\.be\/|v\/|u\/\w\/|embed\/|live\/|shorts\/|watch\?v=|&v=)([^#&?]*).*/)))) && (
                  <div 
                    ...
                    onDoubleClick={(e) => {
                      ...
                      const current = pendingSeekTargetRef.current !== null ? pendingSeekTargetRef.current : (typeof playerRef.current.getCurrentTime === 'function' ? (playerRef.current.getCurrentTime() || 0) : 0);
                      const duration = typeof playerRef.current.getDuration === 'function' ? (playerRef.current.getDuration() || 0) : 0;
