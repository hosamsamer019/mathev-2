...
class AnalyticsService {
    static async getOverview(userId) {
        const examStats = await analytics_repository_js_1.AnalyticsRepository.getExamStats(userId);
        const hwStats = await analytics_repository_js_1.AnalyticsRepository.getHomeworkStats(userId);
    ...
    static async getCharts(userId) {
        const exams = await analytics_repository_js_1.AnalyticsRepository.getRecentExams(userId, 10);
        const homeworks = await analytics_repository_js_1.AnalyticsRepository.getRecentHomeworks(userId, 10);
        return {
    ...
    static async getRecentActivities(userId, limit = 10) {
        const exams = await analytics_repository_js_1.AnalyticsRepository.getRecentExams(userId, limit);
        const homeworks = await analytics_repository_js_1.AnalyticsRepository.getRecentHomeworks(userId, limit);
        const merged = [
            ...exams.map(e => ({
                title: e.exam.title,
                type: 'امتحان',
    ...
    static async getTeacherOverview(teacherId) {
        const stats = await analytics_repository_js_1.AnalyticsRepository.getTeacherStats(teacherId);
